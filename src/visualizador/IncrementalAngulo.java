...
class IncrementalAngulo extends Triangulacion{
   ...
   // Inserta el vértice "v" y sus correspondientes Aristas en la triangulación
   {
      ...
      trianguloInicial();
        ...
        for (i=3; i<numPuntos; i++)
        {
          ...
          if (listaAristas [numAristas-1].obtenerDest().menorQue(listaAristas [numAristas-1].obtenerOrig(), angulo) )
          {
   ...
   // tamaño de ese array si fuera necesario
	 ...
	 capacidadPuntos += INIT;
      ...
      else
      {
          ...
          {
               if (vi.menorQue(listaPuntos[i], angulo) && !insertado)
               {
                   temp[j] = new Punto (vi);
                   j++;
                   insertado = true;
